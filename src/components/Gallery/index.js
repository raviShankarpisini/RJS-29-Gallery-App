...
class Gallery extends Component {
  state = {
    ...
    mainImageAltText: imagesList[0].imageAltText,
  }

  clickThumbnail = (imageUrl, imageAltText) => {
    this.setState({
      mainImageUrl: imageUrl,
      mainImageAltText: imageAltText,
    })
  }

  render() {
    const {mainImageUrl, mainImageAltText} = this.state
          ...
          <ul className="list-items">
            {imagesList.map(eachImageDetails => (
              <ThumbnailItem
                ...
                isThumbnailClicked={eachImageDetails.imageUrl === mainImageUrl}
              />
            ))}
          </ul>
        </div>
